...
return vimify.loadToFirstOrLast();}};vimify.currentItem=function(){var current,items;current=null;items=$(vimify.selectors.item).sort(function(a,b){return $(a).offset().top<$(b).offset().top;});items.each(function(_,item){item=$(item);if(item.offset().top<=$(window).scrollTop()){return current||(current=item);}});return current;};vimify.nextItem=function(){var current;current=vimify.currentItem();if(current){return current.next(vimify.selectors.item);}else{return $(vimify.selectors.item).first();}};vimify.previousItem=function(){var current;current=vimify.currentItem();if(current===null||current.offset().top<$(window).scrollTop()-50){return current;}else{return current.prev();}};vimify.nextPageLink=function(){if(vimify.selectors.nextPage){return $(vimify.selectors.nextPage);}else{return $("[rel=next]");}};vimify.prevPageLink=function(){if(vimify.selectors.prevPage){return $(vimify.selectors.prevPage);}else{return $("[rel=prev]");}};vimify.loadToFirstOrLast=function(){if(window.location.hash==="#first"){return $(window).load(function(){return $(window).scrollTop($(vimify.selectors.item).first().offset().top);});}else if(window.location.hash==="#last"){return $(window).load(function(){return $(window).scrollTop($(vimify.selectors.item).last().offset().top);});}};vimify.j=function(e){var link,next;if(!vimify.hasManyItems()){return;}
