...
vimify.selectors={};vimify.registeredKeys={};vimify.initJko(opts);vimify.initComment(opts);vimify.initSearch(opts);return vimify.initHelp(opts);};vimify.register=function(key,fn,help){vimify.registeredKeys[key]={keys:[key],fn:fn,help:help};return keypress.combo(key,fn);};vimify.registerAlias=function(aliasKey,realKey){var keyData;keyData=vimify.registeredKeys[realKey];keyData.keys.push(aliasKey);return keypress.combo(aliasKey,keyData.fn);};vimify.hasManyItems=function(){return $(vimify.selectors.item).length>1;};vimify.initComment=function(opts){if(opts["comment"]){vimify.selectors.comment=opts["comment"];vimify.register("c",vimify.c,"Comment");if(opts["advancedKeys"]){vimify.registerAlias("i","c");}
...
vimify.register("h",vimify.h,"Show/hide help");if(opts["advancedKeys"]){return vimify.registerAlias("?","h");}};vimify.h=function(){if($(".vimify-help").length>0){vimify.closeHelp();return;}
...
e.preventDefault();link=vimify.currentItem().find(vimify.selectors.itemLink);return window.location=link.attr("href");};vimify.initSearch=function(opts){if(opts["search"]){vimify.selectors.search=opts["search"];return vimify.register("s",vimify.s,"Search");}};vimify.s=function(){var search;search=$(vimify.selectors.search);$(window).scrollTop(search.offset().top-20);return search.focus();};