...
return vimify.loadToFirstOrLast();}};vimify.currentItem=function(){var current,items;current=null;items=$(vimify.selectors.item).sort(function(a,b){return $(a).offset().top<$(b).offset().top;});items.each(function(_,item){item=$(item);if(item.offset().top<=$(window).scrollTop()){return current||(current=item);}});return current;};vimify.nextItem=function(){var current;current=vimify.currentItem();if(current){return current.next(vimify.selectors.item);}else{return $(vimify.selectors.item).first();}};vimify.previousItem=function(){var current;current=vimify.currentItem();if(current===null||current.offset().top<$(window).scrollTop()-50){return current;}else{return current.prev();}};vimify.nextPageLink=function(){if(vimify.selectors.nextPage){return $(vimify.selectors.nextPage);}else{return $("[rel=next]");}};vimify.prevPageLink=function(){if(vimify.selectors.prevPage){return $(vimify.selectors.prevPage);}else{return $("[rel=prev]");}};vimify.loadToFirstOrLast=function(){if(window.location.hash==="#first"){return $(window).load(function(){return vimify.j();});}else if(window.location.hash==="#last"){return $(window).load(function(){return $(window).scrollTop($(vimify.selectors.item).last().offset().top);});}};vimify.j=function(){var link,next;if(!vimify.hasManyItems()){return;}
...
prev=vimify.previousItem();if(prev&&prev.length>0){return $(window).scrollTop(prev.offset().top);}else{link=vimify.prevPageLink();if(link.length>0){return window.location=link.attr("href")+"#last";}else{return $(window).scrollTop(0);}}};vimify.o=function(){var link;if(!vimify.hasManyItems()){return;}
link=vimify.currentItem().find(vimify.selectors.itemLink);return window.location=link.attr("href");};vimify.initSearch=function(opts){if(opts["search"]){vimify.selectors.search=opts["search"];keypress.combo("s",vimify.s);if(opts["advancedKeys"]){return keypress.combo("/",vimify.s);}}};vimify.s=function(){var search;search=$(vimify.selectors.search);$(window).scrollTop(search.offset().top-20);return search.focus();};